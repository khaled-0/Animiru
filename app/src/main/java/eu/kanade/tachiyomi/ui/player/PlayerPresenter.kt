...
class PlayerPresenter(
    ...
    val episodeList by lazy {
        ...
        val episodesForPlayer = when {
            preferences.skipSeen() || preferences.skipFiltered() -> {
                val filteredEpisodes = dbEpisodes.filterNot {
                    when {
                        preferences.skipSeen() && it.seen -> true
                        preferences.skipFiltered() -> {
                            anime.seenFilter == Anime.EPISODE_SHOW_SEEN && !it.seen ||
                                ...
                                anime.filleredFilter == Anime.EPISODE_SHOW_FILLERED && !it.filler ||
                                anime.filleredFilter == Anime.EPISODE_SHOW_NOT_FILLERED && it.filler
                        }
                        else -> false
                    }
                }

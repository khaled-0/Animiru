        ...
        preferenceCategory {
            ...
            preference {
                ...
                fun updateSummary() {
                    ...
                    val allExcluded = excludedCategories.size == categoriesAnime.size

                    val includedItemsText = when {
                        // Some selected, but not all
                        includedCategories.isNotEmpty() && includedCategories.size != categoriesAnime.size -> includedCategories.joinToString { it.name }
                        // All explicitly selected
                        includedCategories.size == categoriesAnime.size -> context.getString(R.string.all)
    ...
    class AnimelibGlobalUpdateCategoriesDialog : DialogController() {
        ...
        override fun onCreateDialog(savedViewState: Bundle?): Dialog {
            ...
            var selected = categories
                .map {
                    when (it.id.toString()) {
                        in preferences.animelibUpdateCategories()
                            .get(),
                        -> QuadStateTextView.State.CHECKED.ordinal
                        in preferences.animelibUpdateCategoriesExclude()
                            .get(),

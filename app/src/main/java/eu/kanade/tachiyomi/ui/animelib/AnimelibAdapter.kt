...
/**
 * This adapter stores the categories from the animelib, used with a ViewPager.
 *
...
class AnimelibAdapter(
    ...
    override fun bindView(view: View, position: Int) {
        (view as AnimelibCategoryView).onBind(categories[position])
        boundViews.add(view)
    }
    ...
    override fun recycleView(view: View, position: Int) {
        (view as AnimelibCategoryView).onRecycle()
    ...
    override fun getItemPosition(obj: Any): Int {
        val view = obj as? AnimelibCategoryView ?: return POSITION_NONE
    ...
    fun onDestroy() {
        for (view in boundViews) {
            if (view is AnimelibCategoryView) {
                view.onDestroy()
            }
        }
    }
